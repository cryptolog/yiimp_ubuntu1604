...
################################################################################
#
#   Script to automate the install of a yiimp mining pool
#
################################################################################
output() {
    printf "\E[0;33;40m"
...
displayErr() {
    ...
    output " "
    output "Verify your answers before proceeding for each line"
    output " "
    ...
    read -e -p "Enter a new location for /site/adminRights this is to customize the Administrative Panel entrance url (such as myAdminpanel) : " admin_panel
    read -e -p "Enter the Public IP of the system you will use to access the administrative panel (http://www.whatsmyip.org/) : " Public
...
' | sudo -E tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    # Installing pre-reqs to compile coins
    output " "
    output "Installing pre-reqs to compile coins"
    output " "
    sleep 5
    ...
    output " "
    output "Installing Fail2Ban & UFW"
    output " "
    sleep 3
...
' | sudo -E tee /etc/nginx/sites-available/$server_name.conf >/dev/null 2>&1
    ...
    # Installing SSL (with SubDomain)
    output " "
    output "Installing LetsEncrypt and setting up SSL (with SubDomain)"
    output " "
...
' | sudo -E tee /etc/nginx/sites-available/$server_name.conf >/dev/null 2>&1
    ...
    # Installing SSL (without SubDomain)
    output " "
    output "Installing LetsEncrypt and setting up SSL (without SubDomain)"
    output " "
    sleep 3
...
' | sudo -E tee /etc/yiimp/keys.php >/dev/null 2>&1
    ...
    # Yiimp serverconfig.php setup
    output " "
    output "Yiimp serverconfig.php setup"
    output " "
    sleep 5
...
' | sudo -E tee /var/web/serverconfig.php >/dev/null 2>&1
    ...
    # Setting up final directory permissions
    output " "
    output "Setting up final directory permissions"
    output " "
    sleep 3
    ...
    output " "
    output " "
    output "Mysql information is saved in ~/.my.cnf."
    output " "
    output "Wallet addresses need to be in the /var/web/serverconfig.php file."
    output " "
    output "Also add your public and private keys."
    output " "
    output " "
    output " "
